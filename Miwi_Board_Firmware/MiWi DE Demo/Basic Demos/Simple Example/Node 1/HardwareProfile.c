...
void BoardInit(void)
{
        ...
        LCDInit();
    #elif defined(CCLEE_CUSTOM_PIC18F25K20)
        WDTCONbits.SWDTEN = 0; 		//disable WDT
	...
	while (!(OSCCON & 0x40));	//wait till clock stable
		...
		// configure led output and channel input
		ANSEL = 0x1F;			//RA0,1,2,3,4 as digital port
		TRISA = 0xFC;			//RA0, 1 as output, 2,3,4,5,6,7 as input
