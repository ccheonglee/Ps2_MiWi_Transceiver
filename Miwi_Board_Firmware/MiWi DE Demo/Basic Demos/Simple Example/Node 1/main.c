...
#endif
{
    ...
    // Temporary enable this to ensure USART TX is working.
    //DemoOutput_Greeting();
    ...
    while(1)
    {
        ...
        {
            // Check if data receive via USART
            //if (ConsoleIsGetReady() == 0)
            //    continue;

			// Check if data receive vis PS2 controller
			Response = get_psx();
			if (Response != 1){
				// no response from ps2 controller, you might need to do some handling here
				continue;		
			}
			
			// if there is response, get the data returned by controller, and send to other side of miwi
            ...
            /*******************************************************************/
            MiApp_FlushTx();
            MiApp_WriteData(psxData.COMBO1);
			MiApp_WriteData(psxData.COMBO2);
			MiApp_WriteData(psxData.RJOY_X);
			MiApp_WriteData(psxData.RJOY_Y);
			MiApp_WriteData(psxData.LJOY_X);
			MiApp_WriteData(psxData.LJOY_Y);
